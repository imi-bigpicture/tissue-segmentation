...
def test_create_tissue_mask():
    ...
    #   (can be adjusted using dilation_disk_size parameter)
    image = io.imread(os.path.join(os.path.dirname(__file__), "data", "29.jpg"))
    mask = create_tissue_mask(image)
    mask = 255 * mask.astype(np.uint8)
    mask_ref = io.imread(
        os.path.join(os.path.dirname(__file__), "data", "29_mask_ref.png")
    )
